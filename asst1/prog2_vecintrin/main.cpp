...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        int width = std::min(VECTOR_WIDTH, N - 1);
        ...
        _cs149_vgt_int(maskExpPositive, y, zero, maskAll);

        while (_cs149_cntbits(maskExpPositive) > 0) {
            _cs149_vmult_float(result, result, x, maskExpPositive);
            _cs149_vadd_int(count, count, one, maskExpPositive);
            _cs149_vlt_int(maskExpPositive, count, y, maskAll);
        }
