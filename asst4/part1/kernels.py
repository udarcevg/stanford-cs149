...
def matrix_transpose(a_tensor):
    ...
    for i in nl.affine_range(M // tile_dim):
        for j in nl.affine_range(N // tile_dim):
            ...
            a_tile = nl.ndarray(
                (tile_dim, tile_dim), dtype=a_tensor.dtype, buffer=nl.sbuf
            )
            nisa.dma_copy(
                src=a_tensor[
            ...
            out_tile = nl.ndarray(
                (tile_dim, tile_dim), dtype=a_tile.dtype, buffer=nl.sbuf
            )

            nisa.tensor_copy(
                src=transposed_psum, dst=out_tile
            )

            nisa.dma_copy(
                src=out_tile, dst=out[
                    j * tile_dim:(j + 1) * tile_dim,
                    i * tile_dim:(i + 1) * tile_dim
                ]
            )
